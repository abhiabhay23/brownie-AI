...
const initialDefaultMessages: Message[] = [
  {
    id: "1",
    sender: "bot",
...
const [messages, setMessages] = useState<Message[]>(initialDefaultMessages);
const [isLoaded, setIsLoaded] = useState(false);

// 1. Load saved messages from the browser when the page opens
useEffect(() => {
  const savedChat = localStorage.getItem("brownie_chat_history");
  if (savedChat) {
    try {
...
// 2. Save messages to the browser automatically whenever they change
useEffect(() => {
  if (isLoaded) {
    localStorage.setItem("brownie_chat_history", JSON.stringify(messages));
  }
}, [messages, isLoaded]);
...
const handleClearChat = () => {
  if (confirm("Are you sure you want to clear your chat history?")) {
    setMessages(initialDefaultMessages);
    localStorage.removeItem("brownie_chat_history");
  }
};
